...
public class RequestThread extends Thread {
	...
	private void sendHeader(OutputStream out, int code, String contentType,
		...
		Log.v("org.WifiConnect", "Returning header\n" + header);
	...
	public void run() {
		...
		try {
			...
			if (path.startsWith("/packages/")) {
				...
				Log.v("org.WifiConnect", "Serving file " + file);
				...
				Log.v("org.WifiConnect", "Serving asset " + path.substring(1));
			}
			...
			while ((bytesRead = content.read(buffer)) != -1) {
				Log.v("org.WifiConnect", "read " + bytesRead + " bytes");
				if (bytesRead > 0) {
					out.write(buffer, 0, bytesRead);
					Log.v("org.WifiConnect", "written");
				}
			}
			_socket.shutdownInput();
			_socket.shutdownOutput();
			Log.v("org.WifiConnect", "Done");
		} catch (NameNotFoundException e) {
			try {
				sendError(out, 404, "File Not Found.");
			} catch (IOException e1) {
				Log.v("org.WifiConnect", e1.getMessage(), e1);
			}
