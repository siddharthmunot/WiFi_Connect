...
import org.WifiConnect.servaldna.meshms.MeshMSMessage;
import org.WifiConnect.servaldna.meshms.MeshMSMessageList;
import org.WifiConnect.ui.SimpleAdapter;
...
public class ShowConversationActivity extends ListActivity implements OnClickListener, SimpleAdapter.ViewBinder<Object>, IPeerListListener {
	...
	private void sendMessage() {
		...
		if ((message == null) && (msg == null)) {
			Log.i(TAG, "Inside sendmessage, returning 2..");
			return;
		}
		...
		if (message != null) {
			...
			Log.i(TAG, "messageText: " + messageText + " message: " + message);
		}

		if (msg != null) {
			Log.i(TAG, "Inside msg..");
			messageText = msg;
		}

		Log.i(TAG, "messageText: " + messageText);
	...
	public void bindView(int position, Object object, View view) {
		...
		if (object instanceof MeshMSMessage) {
			MeshMSMessage meshMSMessage = (MeshMSMessage) object;
			messageText.setText(Html.fromHtml(meshMSMessage.text));
		}else {
			messageText.setText(Html.fromHtml(object.toString()));
		}
