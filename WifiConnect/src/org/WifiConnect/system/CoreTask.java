	...
	public void killProcess(Shell shell, String processName)
		...
		while ((pid = getPid(processName)) >= 0) {
			if (timeout <= SystemClock.elapsedRealtime()) {
				Log.v("org.WifiConnect", "Giving up");
				break;
			}
			if (pid != lastPid) {
				try {
					Log.v("org.WifiConnect", "Killing " + processName + " pid "
							+ pid);
					CommandLog c = new CommandLog("kill " + pid);
					shell.add(c);
					c.exitCode();
				} catch (Exception e) {
					Log.v("org.WifiConnect", "kill failed", e);
				}
			}
			lastPid = pid;
			try {
			...
			while ((ent = str.getNextEntry()) != null) {
				try {
					...
					if (ent.isDirectory()) {
						if (!file.exists())
							if (!file.mkdirs())
								Log.v("org.WifiConnect", "Failed to create path "
										+ filename);
					} else {
						if (extract == null || extract.contains(filename)) {
							...
							if (filename.indexOf("bin/") >= 0
								...
								shell.add(new CommandLog("chmod 755", file
										.getCanonicalPath()));
							Log.v("org.WifiConnect", "Extracted " + filename);
						}
					}
				} catch (Exception e) {
					Log.v("org.WifiConnect", e.toString(), e);
				}
				str.closeEntry();
			}
		} finally {
